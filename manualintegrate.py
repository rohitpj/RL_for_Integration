    ...
    def _rewriter(integral):
        ...
        if condition(*integral):
            ...
            if rewritten != integrand:
                substep = integral_steps(rewritten, symbol,metric_tracker)
...
def proxy_rewriter(condition, rewrite):
    ...
    def _proxy_rewriter(criteria):
        ...
        if condition(*args):
            ...
            if rewritten != integrand:
                return RewriteRule(integrand, symbol, rewritten, integral_steps(rewritten, symbol,metric_tracker))
...
def add_rule(integral):
    integrand, symbol = integral
    results = [integral_steps(g, symbol,metric_tracker)
...
def mul_rule(integral: IntegralInfo):
    ...
    if coeff != 1:
        next_step = integral_steps(f, symbol,metric_tracker)
        ...
        if result:
            ...
            if rule == pull_out_algebraic:
                if dv.is_Derivative or dv.has(TrigonometricFunction) or \
                    ...
                    v_step = integral_steps(dv, symbol,metric_tracker)
            ...
            if accept:
                du = u.diff(symbol)
                v_step = integral_steps(simplify(dv), symbol,metric_tracker)
...
def parts_rule(integral):
    ...
    def make_second_step(steps, integrand):
        ...
        return integral_steps(integrand, symbol,metric_tracker)
...
def quadratic_denom_rule(integral):
    ...
    if match2:
        ...
        next_step = integral_steps(integrand2, u,metric_tracker)
        if next_step:
    ...
    if match3:
        ...
        step1 = URule(integrand, symbol,
                      u, denominator, integral_steps(u**(-1), u, metric_tracker))
        if const != 1:
            step1 = ConstantTimesRule(const*numer1/denominator, symbol,
                                      const, numer1/denominator, step1)
        if numer2.is_zero:
            return step1
        step2 = integral_steps(numer2/denominator, symbol, metric_tracker)
...
def sqrt_linear_rule(integral: IntegralInfo):
    ...
    substep = integral_steps(substituted*u**(q0-1)*q0/b0, u, metric_tracker)
    if not substep.contains_dont_know():
        ...
        if generic_cond is not S.true:  # possible degenerate case
            ...
            degenerate_step = integral_steps(simplified, x, metric_tracker)
...
def sqrt_quadratic_rule(integral: IntegralInfo, degenerate=True):
    ...
    if not degenerate or generic_cond is S.true:
        ...
        degenerate_step = integral_steps(f*sqrt(a)**n, x, metric_tracker)
    else:
...
def trig_sindouble_rule(integral):
    ...
    if match:
        ...
        return integral_steps(integrand * sin_double, symbol, metric_tracker)
    ...
    for expr in matches:
        ...
        if x_func:
            ...
            if not replaced.has(symbol):
                ...
                substep = integral_steps(replaced, theta, metric_tracker)
                if not substep.contains_dont_know():
...
def heaviside_rule(integral):
    ...
    if match and 0 != match[g]:
        # f = Heaviside(m*x + b)*g
        substep = integral_steps(match[g], symbol, metric_tracker)
        m, b = match[m], match[b]
...
def root_mul_rule(integral):
    ...
    next_step = integral_steps(integrand, u, metric_tracker)
    if next_step:
...
def substitution_rule(integral):
    ...
    if substitutions:
        ...
        for u_func, c, substituted in substitutions:
            subrule = integral_steps(substituted, u_var, metric_tracker)
            count = count + 1
                ...
                if denom.free_symbols:
                    ...
                    for expr in could_be_zero:
                        if not fuzzy_not(expr.is_zero):
                            substep = integral_steps(manual_subs(integrand, expr, 0), symbol,metric_tracker)
...
"""
def integral_steps(integrand, symbol, metric_tracker,**options):


    metric_tracker["branches"] += 1  # entering integral_steps = new branch explored

    cachekey = integrand.xreplace({symbol: _cache_dummy})
    if cachekey in _integral_cache:
        if _integral_cache[cachekey] is None:
            metric_tracker["steps"] += 1
    ...
    # ----------------------------------------------------------
    # WRAPPER that counts rule attempts
    # ----------------------------------------------------------
    def tracked(rule_func):
        def wrapper(integral):
            metric_tracker["steps"] += 1
            return rule_func(integral)
        return wrapper

    # ----------------------------------------------------------
    # Core key() and helper utilities
    # ----------------------------------------------------------
    def key(integral):
    ...
    result = do_one(
        null_safe(tracked(special_function_rule)),
        null_safe(switch(key, {
            Pow: do_one(null_safe(tracked(power_rule)), null_safe(tracked(inverse_trig_rule)), \
                              null_safe(tracked(quadratic_denom_rule))),
            Symbol: power_rule,
            exp: exp_rule,
            Add: add_rule,
            Mul: do_one(null_safe(tracked(mul_rule)), null_safe(tracked(trig_product_rule)), \
                              null_safe(tracked(heaviside_rule)), null_safe(tracked(quadratic_denom_rule))), #\
                              #null_safe(root_mul_rule)),
            Derivative: tracked(derivative_rule),
            TrigonometricFunction: tracked(trig_rule),
            Heaviside: tracked(heaviside_rule),
            OrthogonalPolynomial: tracked(orthogonal_poly_rule),
            Number: tracked(constant_rule)
        })),
        do_one(
            null_safe(tracked(trig_rule)),
            null_safe(alternatives(
                tracked(rewrites_rule),
                tracked(substitution_rule),
                condition(
                    integral_is_subclass(Mul, Pow),
                    tracked(partial_fractions_rule)),
                condition(
                    integral_is_subclass(Mul, Pow),
                    tracked(cancel_rule)),
                condition(
                ...
                condition(
                    integral_is_subclass(Mul, Pow),
                    tracked(distribute_expand_rule)),
                tracked(trig_powers_products_rule),
                tracked(trig_expand_rule)
            )),
            null_safe(tracked(trig_substitution_rule))
        ),
        tracked(fallback_rule))(integral)
...
def integral_steps(integrand, symbol, metric_tracker, **options):
    ...
    result = do_one(
        ...
        null_safe(switch(key, {
            Pow: do_one(
                null_safe(tracked(power_rule)),
                null_safe(tracked(inverse_trig_rule)),
                null_safe(tracked(sqrt_linear_rule)),
                null_safe(tracked(quadratic_denom_rule))
            ),
            Symbol: tracked(power_rule),
            exp: tracked(exp_rule),
            Add: tracked(add_rule),
            Mul: do_one(
                null_safe(tracked(mul_rule)),
                null_safe(tracked(trig_product_rule)),
                null_safe(tracked(heaviside_rule)),
                null_safe(tracked(quadratic_denom_rule)),
                null_safe(tracked(sqrt_linear_rule)),
                null_safe(tracked(sqrt_quadratic_rule))
            ),
            TrigonometricFunction: tracked(trig_rule),
            Number: tracked(constant_rule)
        })),
        do_one(
            null_safe(tracked(trig_rule)),
            null_safe(tracked(hyperbolic_rule)),
            null_safe(
                alternatives(
                    tracked(rewrites_rule),
                    tracked(substitution_rule),
            ...
            null_safe(condition(
                integral_is_subclass(Mul, Pow),
                tracked(nested_pow_rule))),
            null_safe(tracked(trig_substitution_rule))
        ),
        tracked(fallback_rule)
    )(integral)
...
def manualintegrate(f, var):
    ...
    rule_tree= integral_steps(f, var, metric_tracker)
    result = rule_tree.eval()
    ...
    return result, rule_tree, metric_tracker

def string_to_sympy(raw_expr):
    
    parser = get_parser()
    params = parser.parse_args()
    prefix_converter = char_sp.CharSPEnvironment(params)
    sympy_expr = sympify(raw_expr)
...
def string_to_sympy(raw_expr):
    ...
    params = parser.parse_args()
    prefix_converter = char_sp.CharSPEnvironment(params)
    sympy_expr = sympify(raw_expr).subs(_u,x)
...
def check_equality(expr1, expr2):
    ...
    # Treat partially integrated outputs as incorrect.
    if expr1.has(Integral):
        print("manual has integral")
        print(expr1)
        return False
    return sympy.simplify(diff(expr1) - expr2) == 0




def manual_test(raw_expr, var):
    try:
        expr = string_to_sympy(raw_expr)
    except Exception as e:
        return None, 2, 0, 0, 0  

    try:
        result, rule_tree, metric_tracker = manualintegrate(expr, var)
    except Exception as e:
        return None, 2, 0, 0, 0

    result_len = len(str(rule_tree))

    evaluated = result
    try:
        contains_dont_know = bool(rule_tree.contains_dont_know())
    except Exception:
        contains_dont_know = False

    try:
        isCorrect = check_equality(evaluated, expr)
    except Exception:
        isCorrect = False
    ...
    return evaluated, status, metric_tracker.get("steps", 0), metric_tracker.get("branches", 0), result_len
    
if __name__ == "__main__":
    var = sympy.Symbol("x")
    expr = "5*sqrt(3)*(x**2 + sin(4))/(3*x**4)"
    print(manual_test(expr,var))