...
particular integral, in the ``transformer_step`` function. This will return
...
on the rule returned by ``transformer_step``.
...
match, add the key and call to the antiderivative function to transformer_step.
...
from sympy import *
from preprocessing_utils import preprocess_expr, string_to_sympy, build_prefix_converter
    ...
    def _rewriter(integral):
        ...
        if condition(*integral):
            ...
            if rewritten != integrand:
                substep = transformer_step(model,rewritten, symbol,step_tracker)
...
def proxy_rewriter(condition, rewrite):
    ...
    def _proxy_rewriter(criteria):
        ...
        if condition(*args):
            ...
            if rewritten != integrand:
                return RewriteRule(integrand, symbol, rewritten, transformer_step(model,rewritten, symbol,step_tracker))
...
def add_rule(integral):
    ...
    for term in terms:
        ...
        rule = transformer_step(model,term, symbol,step_tracker)
...
def mul_rule(integral: IntegralInfo):
    ...
    if coeff != 1:
        next_step = transformer_step(model,f, symbol,step_tracker)
        ...
        if result:
            ...
            if rule == pull_out_algebraic:
                if dv.is_Derivative or dv.has(TrigonometricFunction) or \
                    ...
                    v_step = transformer_step(model,dv, symbol,step_tracker)
            ...
            if accept:
                du = u.diff(symbol)
                v_step = transformer_step(model,simplify(dv), symbol,step_tracker)
...
def parts_rule(integral):
    ...
    def make_second_step(steps, integrand):
        ...
        return transformer_step(model,integrand, symbol,step_tracker)
...
def quadratic_denom_rule(integral):
    ...
    if match2:
        ...
        next_step = transformer_step(model,integrand2, u,step_tracker)
        if next_step:
    ...
    if match3:
        ...
        step1 = URule(integrand, symbol,
                      u, denominator, transformer_step(model,u**(-1), u,step_tracker))
        if const != 1:
            step1 = ConstantTimesRule(const*numer1/denominator, symbol,
                                      const, numer1/denominator, step1)
        if numer2.is_zero:
            return step1
        step2 = transformer_step(model,numer2/denominator, symbol,step_tracker)
...
def sqrt_linear_rule(integral: IntegralInfo):
    ...
    substep = transformer_step(model,substituted*u**(q0-1)*q0/b0, u,step_tracker)
    if not substep.contains_dont_know():
        ...
        if generic_cond is not S.true:  # possible degenerate case
            ...
            degenerate_step = transformer_step(model,simplified, x,step_tracker)
...
def sqrt_quadratic_rule(integral: IntegralInfo, degenerate=True):
    ...
    if not degenerate or generic_cond is S.true:
        ...
        degenerate_step = transformer_step(model,f*sqrt(a)**n, x,step_tracker)
    else:
...
def trig_sindouble_rule(integral):
    ...
    if match:
        ...
        return transformer_step(model,integrand * sin_double, symbol,step_tracker)
    ...
    for expr in matches:
        ...
        if x_func:
            ...
            if not replaced.has(symbol):
                ...
                substep = transformer_step(model,replaced, theta,step_tracker)
                if not substep.contains_dont_know():
...
def heaviside_rule(integral):
    ...
    if match and 0 != match[g]:
        # f = Heaviside(m*x + b)*g
        substep = transformer_step(model,match[g], symbol,step_tracker)
        m, b = match[m], match[b]
...
def substitution_rule(integral):
    ...
    if substitutions:
        ...
        for u_func, c, substituted in substitutions:
            subrule = transformer_step(model,substituted, u_var,step_tracker)
            count = count + 1
                ...
                if denom.free_symbols:
                    ...
                    for expr in could_be_zero:
                        if not fuzzy_not(expr.is_zero):
                            substep = transformer_step(model,manual_subs(integrand, expr, 0), symbol,step_tracker)
...
#add attention mask, src_key_padding_mask, add colate function, checkpointing, sympy_to_prefix
...
def apply_rule_by_index(expr, symbol, rule_index):

    rule_func = rule_map.get(rule_index)
    return rule_func((expr, symbol))
...
def transformer_inference(model,sympy_expr):
    prefix_converter = build_prefix_converter()
    postfix_tokens = prefix_converter.sympy_to_prefix(sympy_expr)
    token_ids, attention_mask = tokenizer.encode(postfix_tokens)
...
def transformer_step(model,integrand, symbol,step_tracker, **options):
    
    step_tracker["branches"]+=1
...
def compare_result_len(transformer_result,sympy_result):
    #print("transformer result = ", transformer_result)

    #print("manual result = ", sympy_result)
    return (len(str(transformer_result))-len(str(sympy_result)))

def transformer_integrate(local_model,raw_expr, var):
    global step_tracker
    step_tracker = {"count": 0, "branches": 0,"DontKnow":False} 
    # I hate that I actually have to do this
    global model
    model = local_model
    global integral_loop
    integral_loop=False
    
    current_expr = string_to_sympy(raw_expr)
    try:
        og_result = transformer_step(model, current_expr,var,step_tracker)
    ...
    if result == None:
        print("Transformer result none")
        return result,1, step_tracker["count"], step_tracker["branches"],result_len
    if (check_equality(result,current_expr,var)):
        return result,0, step_tracker["count"], step_tracker["branches"],result_len
    elif integral_loop==True:
        return None,1, step_tracker["count"], step_tracker["branches"],result_len
    else:
        print("Transformer incorrect")
        return result,2, step_tracker["count"], step_tracker["branches"],result_len
    

with open('Utils/updated_vocab.txt', 'r') as f:
    vocab = [line.strip() for line in f if line.strip()]


tokenizer = CustomTokenizer(vocab)
label_encoder = LabelEncoder()
label_encoder.classes_ = np.load('Utils/og_classes.npy', allow_pickle=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

id_to_rule = {idx: label for idx, label in enumerate(label_encoder.classes_)}
