...
def initialize_runtime(
    ...
    actor_path='Models/updated_rl_94_acc/updated_rl_actor.pt',
    ...
    print("using actor from ", actor_path)
    agent.actor = torch.load(actor_path, map_location=device, weights_only=False)
    agent.to(device)

    return agent, None, vocab_size


CONFIG = {
    "dataset":     "Data/csv_out/IBP_test.csv",
    "expr_col":    "function",
    "sample_size": 500,
    "actor_path":  "Models/updated_rl_94_acc/updated_rl_actor.pt",
    "out_csv":     "Data/rl_test_results.csv",
}

SEEDS = [42, 43, 44]

RESULT_MAP = {0: "Correct", 1: "Incorrect", 2: "DontKnowRule", 3: "NoneResult"}


def run_test(seed, agent, full_df, config):
    var = sympy.Symbol("x")
    test_df = full_df.sample(n=min(config["sample_size"], len(full_df)), random_state=seed).reset_index(drop=True)

    agent.eval()
    results = []

    for expr in tqdm.tqdm(test_df[config["expr_col"]].tolist(), desc=f"[seed {seed}]"):
        if has_hyperbolic_trig(sympify(expr)):
            continue
        try:
            rl_out, result_type, result_len, num_attempts, num_branches = rl_integrate(agent, expr, var)
            results.append({
                "expression":           expr,
                "rl_predicted_integral": str(rl_out),
                "rl_status":            RESULT_MAP.get(result_type, "Unknown"),
                "result_length":        result_len,
                "num_rule_attempts":    num_attempts,
                "num_branches_explored": num_branches,
            })
        except Exception as e:
            print(f"Error [{expr}]: {e}")
            results.append({
                "expression":           expr,
                "rl_predicted_integral": None,
                "rl_status":            "Error",
                "result_length":        None,
                "num_rule_attempts":    None,
                "num_branches_explored": None,
            })

    out_df = pd.DataFrame(results)
    suffix = config["out_csv"].replace(".csv", f"_seed{seed}.csv")
    out_df.to_csv(suffix, index=False)

    counts  = out_df["rl_status"].value_counts()
    percent = out_df["rl_status"].value_counts(normalize=True) * 100
    print(f"\n[seed {seed}] Result distribution:")
    print(pd.DataFrame({"count": counts, "percent": percent.round(2)}))
    return out_df


if __name__ == "__main__":
    agent, _, vocab_size = initialize_runtime(actor_path=CONFIG["actor_path"])

    full_df = pd.read_csv(CONFIG["dataset"])
    all_results = []

    for seed in SEEDS:
        out_df = run_test(seed, agent, full_df, CONFIG)
        all_results.append({"seed": seed, "df": out_df})

    print("\n" + "=" * 60)
    print("CROSS-SEED SUMMARY")
    print("=" * 60)
    for entry in all_results:
        acc = (entry["df"]["rl_status"] == "Correct").mean() * 100
        print(f"  Seed {entry['seed']}: {acc:.2f}% correct")

    import numpy as np
    accs = [(entry["df"]["rl_status"] == "Correct").mean() * 100 for entry in all_results]
